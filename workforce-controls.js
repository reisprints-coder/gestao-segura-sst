...
(() => {
  ...
  function consumptionSummary(employeeId, month=''){
    ...
    let qty=0,value=0,deliveries=0,last='';
    rows.forEach(m=>{const sign=m.movement_type==='return'?-1:1;qty+=sign*Number(m.quantity||0);const epi=state.epis.find(e=>e.id===m.epi_id);value+=sign*Number(m.quantity||0)*Number(epi?.unitCost||0);if(m.movement_type==='delivery')deliveries++;if(!last||String(m.movement_at)>last)last=String(m.movement_at);});
    return {qty,value,deliveries,last};
  }
  ...
  function renderEpiConsumption(){
    ...
    const totals=filtered.reduce((a,e)=>{const s=consumptionSummary(e.id,month);a.qty+=s.qty;a.value+=s.value;a.deliveries+=s.deliveries;return a;},{qty:0,value:0,deliveries:0});
    ...
    <div class="workforce-summary"><article class="workforce-card"><small>Quantidade líquida entregue</small><strong>${number(totals.qty)}</strong></article><article class="workforce-card"><small>Entregas registradas</small><strong>${number(totals.deliveries)}</strong></article><article class="workforce-card"><small>Valor estimado consumido</small><strong>${currency(totals.value)}</strong></article><article class="workforce-card"><small>Funcionários no relatório</small><strong>${filtered.filter(e=>consumptionRows(e.id,month).length).length}</strong></article></div>
    <div class="table-card"><div class="table-wrap"><table><thead><tr><th>Funcionário</th><th>CPF</th><th>Entregas</th><th>Qtd. líquida</th><th>Valor estimado</th><th>Última movimentação</th><th>Ações</th></tr></thead><tbody>${filtered.map(e=>{const s=consumptionSummary(e.id,month);if(!s.deliveries&&!s.qty)return '';return `<tr><td><div class="workforce-person-row"><span class="avatar">${initials(e.name)}</span><span><strong>${escapeHtml(e.name)}</strong><small>${escapeHtml(e.registration||'')}</small></span></div></td><td>${escapeHtml(formatCpf(e.cpf))}</td><td>${s.deliveries}</td><td><strong>${number(s.qty)}</strong></td><td>${currency(s.value)}</td><td>${fmtDateTime(s.last)}</td><td><div class="workforce-actions"><button class="btn small" data-workforce-action="view-consumption" data-id="${e.id}">Histórico</button><button class="btn small primary" data-workforce-action="print-epi-sheet" data-id="${e.id}">Ficha EPI</button></div></td></tr>`}).join('')||'<tr><td colspan="7"><div class="workforce-empty">Nenhum consumo registrado para o filtro.</div></td></tr>'}</tbody></table></div></div>`;
  }
  ...
  function renderMaintenanceHistory(){
    ...
    const total=rows.reduce((s,x)=>s+Number(x.cost||0),0);
    const preventative=rows.filter(x=>x.type==='Preventiva').length;
    const corrective=rows.filter(x=>x.type==='Corretiva').length;
    $('#page-content').innerHTML=`<div class="toolbar"><div><h3 style="margin:0">Histórico de Manutenção das Empilhadeiras</h3><p class="muted" style="margin:5px 0 0">Ordens de serviço, custos, tipo de manutenção e situação por equipamento.</p></div><div class="workforce-filters"><select id="wf-maintenance-forklift" class="select"><option value="">Todas as empilhadeiras</option>${state.forklifts.map(f=>`<option value="${f.id}" ${wf.maintenanceForklift===f.id?'selected':''}>${escapeHtml(f.code)} • ${escapeHtml(f.asset||'')}</option>`).join('')}</select><select id="wf-maintenance-status" class="select"><option>Todos</option>${['Aberta','Em andamento','Aguardando peça','Concluída'].map(s=>`<option ${wf.maintenanceStatus===s?'selected':''}>${s}</option>`).join('')}</select><button class="btn" data-workforce-action="print-maintenance">Imprimir</button></div></div>
    <div class="workforce-summary"><article class="workforce-card"><small>Ordens no filtro</small><strong>${rows.length}</strong></article><article class="workforce-card"><small>Preventivas</small><strong>${preventative}</strong></article><article class="workforce-card"><small>Corretivas</small><strong>${corrective}</strong></article><article class="workforce-card"><small>Custo acumulado</small><strong>${currency(total)}</strong></article></div>
    <div class="table-card"><div class="table-wrap"><table><thead><tr><th>Empilhadeira</th><th>Tipo</th><th>Prioridade</th><th>Abertura</th><th>Descrição</th><th>Custo</th><th>Status</th></tr></thead><tbody>${rows.map(x=>`<tr><td><strong>${escapeHtml(x.forklift)}</strong></td><td>${escapeHtml(x.type)}</td><td>${badge(x.priority)}</td><td>${dateBR(x.openedAt)}</td><td>${escapeHtml(x.description)}</td><td>${currency(x.cost)}</td><td>${badge(x.status)}</td></tr>`).join('')||'<tr><td colspan="7"><div class="workforce-empty">Nenhuma manutenção encontrada.</div></td></tr>'}</tbody></table></div></div>`;
  }
  ...
  function renderCoursesEnhanced(){
    const rows=state.courses.filter(c=>(!wf.courseEmployee||c.employeeId===wf.courseEmployee)&&(wf.courseStatus==='Todos'||courseGroup(c)===wf.courseStatus));
    const valid=rows.filter(c=>daysUntil(c.expiresAt)>90).length;
    const expiring=rows.filter(c=>{const d=daysUntil(c.expiresAt);return d>=0&&d<=90}).length;
    const expired=rows.filter(c=>daysUntil(c.expiresAt)<0).length;
    $('#page-content').innerHTML=`<div class="toolbar"><div><h3 style="margin:0">Controle de Cursos dos Funcionários</h3><p class="muted" style="margin:5px 0 0">Matriz individual de treinamentos, certificados e vencimentos.</p></div><div class="workforce-filters"><select id="wf-course-employee" class="select"><option value="">Todos os funcionários</option>${state.employees.map(e=>`<option value="${e.id}" ${wf.courseEmployee===e.id?'selected':''}>${escapeHtml(e.name)}</option>`).join('')}</select><select id="wf-course-status" class="select">${['Todos','Válido','Vence em até 90 dias','Vencido'].map(s=>`<option ${wf.courseStatus===s?'selected':''}>${s}</option>`).join('')}</select><button class="btn" data-workforce-action="print-course-matrix">Imprimir matriz</button><button class="btn primary" data-action="open-form" data-form="course">+ Registrar curso</button></div></div>
    <div class="workforce-summary"><article class="workforce-card"><small>Registros no filtro</small><strong>${rows.length}</strong></article><article class="workforce-card"><small>Válidos</small><strong>${valid}</strong></article><article class="workforce-card"><small>Vencem em até 90 dias</small><strong>${expiring}</strong></article><article class="workforce-card"><small>Vencidos</small><strong>${expired}</strong></article></div>
    <div class="table-card"><div class="table-wrap"><table><thead><tr><th>Funcionário</th><th>Curso</th><th>Instituição</th><th>Realização</th><th>Validade</th><th>Carga horária</th><th>Status</th><th>Ações</th></tr></thead><tbody>${rows.map(c=>{const st=courseStatus(c);return `<tr><td><strong>${escapeHtml(c.employee)}</strong><br><span class="muted">${escapeHtml(c.employeeRegistration)}</span></td><td><strong>${escapeHtml(c.course)}</strong><br><span class="muted">${escapeHtml(c.certificate||'Sem certificado')}</span></td><td>${escapeHtml(c.institution)}</td><td>${dateBR(c.completedAt)}</td><td>${dateBR(c.expiresAt)}</td><td>${Number(c.hours||0)}h</td><td>${badge(st[0],st[1])}</td><td><div class="workforce-actions"><button class="btn small" data-action="open-form" data-form="course" data-id="${c.id}">Editar</button><button class="btn small" data-workforce-action="print-employee-courses" data-id="${c.employeeId}">Imprimir</button>${canHR()?`<button class="btn small danger" data-action="delete" data-collection="courses" data-id="${c.id}">Excluir</button>`:''}</div></td></tr>`}).join('')||'<tr><td colspan="8"><div class="workforce-empty">Nenhum curso encontrado.</div></td></tr>'}</tbody></table></div></div>`;
  }

  function courseGroup(c){ const d=daysUntil(c.expiresAt); if(d<0)return 'Vencido'; if(d<=90)return 'Vence em até 90 dias'; return 'Válido'; }
  ...
  function printMaintenance(){
    ...
    const body=`<h1>HISTÓRICO DE MANUTENÇÃO DE EMPILHADEIRAS</h1><p style="text-align:center">Equipamento: ${escapeHtml(titleFork||'Todas')} • Emitido em ${new Date().toLocaleString('pt-BR')}</p><table><thead><tr><th>Equipamento</th><th>Tipo</th><th>Prioridade</th><th>Abertura</th><th>Descrição</th><th>Custo</th><th>Status</th></tr></thead><tbody>${rows.map(x=>`<tr><td>${escapeHtml(x.forklift)}</td><td>${escapeHtml(x.type)}</td><td>${escapeHtml(x.priority)}</td><td>${dateBR(x.openedAt)}</td><td>${escapeHtml(x.description)}</td><td>${currency(x.cost)}</td><td>${escapeHtml(x.status)}</td></tr>`).join('')||'<tr><td colspan="7">Sem registros.</td></tr>'}</tbody></table><div class="foot">Gestão Segura SST</div>`; printWindow('Histórico de Manutenção',body);
  }

  function printCourseMatrix(){
    const rows=state.courses.filter(c=>(!wf.courseEmployee||c.employeeId===wf.courseEmployee)&&(wf.courseStatus==='Todos'||courseGroup(c)===wf.courseStatus));
    const body=`<h1>MATRIZ DE CURSOS E TREINAMENTOS</h1><p style="text-align:center">${escapeHtml(state.settings.company||'Empresa')} • Emitido em ${new Date().toLocaleString('pt-BR')}</p><table><thead><tr><th>Funcionário</th><th>Matrícula</th><th>Curso</th><th>Instituição</th><th>Realização</th><th>Validade</th><th>C.H.</th><th>Status</th></tr></thead><tbody>${rows.map(c=>`<tr><td>${escapeHtml(c.employee)}</td><td>${escapeHtml(c.employeeRegistration)}</td><td>${escapeHtml(c.course)}</td><td>${escapeHtml(c.institution)}</td><td>${dateBR(c.completedAt)}</td><td>${dateBR(c.expiresAt)}</td><td>${Number(c.hours||0)}h</td><td>${escapeHtml(courseGroup(c))}</td></tr>`).join('')||'<tr><td colspan="8">Sem registros.</td></tr>'}</tbody></table><div class="foot">Gestão Segura SST</div>`; printWindow('Matriz de Cursos',body);
  }
