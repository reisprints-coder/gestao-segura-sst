...
(() => {
  ...
  function generateTermPdf({ code, employee, epi, quantity, reason, movementDate, signedAt, signatureDataUrl, signatureHash }) {
    ...
    y += 10;
    doc.setDrawColor(180); doc.roundedRect(left, y, width, 48, 2, 2);
    ...
    doc.text(`Cargo: ${employee.role || '—'}`, left + 4, y + 26);
    doc.text(`Setor: ${employee.sector || '—'}`, left + 4, y + 32);
    doc.text(`Empresa: ${employee.company || '—'}`, left + 4, y + 38);
    y += 54;
